...
1. Foucsing on writing the function `BuildSincastAtlas`. THe function will take a Sincast object as an input, output an updated Sincast object with newly built Sincast atlas.
2. Updated `SincastSummary` (new entries).
3. Updated Sincast object subset generics `[`, `[[`.
4. Coding style update on `SincastAggregate` and `SincastImpute`.
5. Create `show` method for Sincast atlas.
